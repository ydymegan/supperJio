...
export default function MyJoinedJio() {
    ...
    const handleSubmit = (e) => {
        ...
        ref.doc(selectedJio.jioID).update({joinerID: firebase.firestore.FieldValue.arrayRemove('{user.uid'), order: orderArray}
            )
            .then(() => {
                        ...
                        {filterJio()
                            .map((jio) => (
                                <div key={jio.id} className="jio">
                                    ...
                                    <input type="text" placeholder="Type Yes & Click Remove" onChange={e => setSelectedJio(jio)}></input>
